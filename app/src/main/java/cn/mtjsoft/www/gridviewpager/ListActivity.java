...
public class ListActivity extends AppCompatActivity {
    ...
    private String[] datas = {"美食", "电影", "酒店住宿", "休闲娱乐", "外卖", "自助餐", "KTV", "机票/火车票", "周边游", "美甲美睫",
            "火锅", "生日蛋糕", "甜品饮品", "水上乐园", "汽车服务", "美发", "丽人", "景点", "足疗按摩", "运动健身", "健身", "超市", "买菜",
            "今日新单", "小吃快餐", "面膜", "洗浴/汗蒸", "母婴亲子", "生活服务", "婚纱摄影", "学习培训", "家装", "结婚", "全部分配"};

    private int[] iconS = new int[datas.length];
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ListAdapter adapter = new ListAdapter(Arrays.asList(datas));
        adapter.addHeaderView(getGridViewPager());
        recyclerView.setAdapter(adapter);
    }


    private GridViewPager getGridViewPager(){
        GridViewPager gridViewPager = new GridViewPager(getBaseContext());
        gridViewPager
                // 设置数据总数量
                .setDataAllCount(datas.length)
                // 设置背景色，默认白色
                ...
                .setGridItemClickListener(new GridViewPager.GridItemClickListener() {
                    ...
                    public void click(int position) {
                        Toast.makeText(getBaseContext(), "点击了" + datas[position] + position, Toast.LENGTH_SHORT).show();
                    }
                })
                .show();
